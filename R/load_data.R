...
#   names(finam.stock.list) %>%
#     {which(tolower(name) == tolower(.))} %>%
#     {(finam.stock.list)[.]} %>%
#     as.character %>%
#     as.numeric %>%
#     unique
...
loadStockList <- function (verbose = FALSE, mkts = c(1, 25, 520, 7, 14, 24, 6, 17, 91, 8))
{
  ...
  data <- data.frame(name = names[1:max_len], em = res[1:max_len], market = markets[1:max_len], stringsAsFactors = FALSE)
  data_ <- data[data[, 3] %in% setdiff(mkts, c(1, 3)), ]
  data <- data[data[, 3] != 3, ]
  data <- rbind(data[(data[, 1] %in% data[data[, 3] == 1, 1]) &
                       data[, 3] == 1, ],
                data[!(data[, 1] %in% data[data[, 3] == 1, 1]), ],
                data_)
  return(data)
}
